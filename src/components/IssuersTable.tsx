...
import { IssuerModel, Issuer } from './crds/Issuer';
...
export const IssuersTable: React.FC<IssuersTableProps> = ({ selectedProject }) => {
  ...
  const [items, loaded, loadError] = useK8sWatchResource<Issuer[]>({
  ...
  const rows = (items || []).map((obj) => {
    ...
    const inspectHref = `/cert-manager/inspect/issuers/${ns}/${name}`;
  ...
  return (
    <ResourceTable
      ...
      emptyStateBody={t('No Issuer resources are currently available.')}
